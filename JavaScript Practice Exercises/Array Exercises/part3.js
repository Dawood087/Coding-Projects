...
// 1) What is Madison Marshall's user id? (.filter möglich)
users.find(
...
comments.find((comments) => comments.text === "OK great thanks").userId; // returned nur die UserId

users.find(
  (user) =>
    user.id ===
    comments.find((comments) => comments.text === "OK great thanks").userId
);
...
comments.map((comment) => {
  ...
  return { ...comment, firstName, lastName }; // hängt first-/lastName an das Array dran
});
